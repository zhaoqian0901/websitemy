...
Apache LakehouseDB 是一个基于 MPP 架构的高性能、实时的分析型数据库，以极速易用的特点被人们所熟知，仅需亚秒级响应时间即可返回海量数据下的查询结果，不仅可以支持高并发的点查询场景，也能支持高吞吐的复杂分析场景，这个简短的指南将告诉你如何下载 LakehouseDB 最新稳定版本，在单节点上安装并运行它，包括创建数据库、数据表、导入数据及查询等。

## 下载 LakehouseDB

LakehouseDB 运行在 Linux 环境中，推荐 CentOS 7.x 或者 Ubuntu 16.04 以上版本，同时你需要安装 Java 运行环境（JDK版本要求为8），要检查你所安装的 Java 版本，请运行以下命令：

```
java -version
```

接下来，[下载 LakehouseDB 的最新二进制版本](https://lakehouseDB.apache.org/zh-CN/download)，然后解压。
...
我们进入到 `apache-lakehouseDB-x.x.x/fe` 目录
...
>注意：
>
>这里你可以不配置，默认是在你的LakehouseDB FE 安装目录下的 lakehouseDB-meta，
>
...
你可以通过下面的命令来检查 LakehouseDB 是否启动成功
...
你也可以通过 LakehouseDB FE 提供的Web UI 来检查，在浏览器里输入地址
...
>注意：
>
>1. 这里我们使用 LakehouseDB 内置的默认用户 root 进行登录，密码是空
>2. 这是一个 LakehouseDB 的管理界面，只能拥有管理权限的用户才能登录，普通用户不能登录。

#### 连接 FE

我们下面通过 MySQL 客户端来连接 LakehouseDB FE，下载免安装的 [MySQL 客户端](https://lakehouseDB-build-hk.oss-cn-hongkong.aliyuncs.com/mysql-client/mysql-5.7.22-linux-glibc2.12-x86_64.tar.gz)

解压刚才下载的 MySQL 客户端，在 `bin/` 目录下可以找到 `mysql` 命令行工具。然后执行下面的命令连接 LakehouseDB。
...
>注意：
>
>1. 这里使用的 root 用户是 lakehouseDB 内置的默认用户，也是超级管理员用户，具体的用户权限查看 [权限管理](../admin-manual/privilege-ldap/user-privilege.md)
>2. -P ：这里是我们连接 LakehouseDB 的查询端口，默认端口是 9030，对应的是fe.conf里的 `query_port`
...
LakehouseDB支持基于SSL的加密连接，当前支持TLS1.2，TLS1.3协议，可以通过以下配置开启LakehouseDB的SSL模式：
修改FE配置文件`conf/fe.conf`，添加`enable_ssl = true`即可。

接下来通过`mysql`客户端连接LakehouseDB，mysql支持五种SSL模式：
...
LakehouseDB开启SSL加密连接需要密钥证书文件验证，默认的密钥证书文件位于`LakehouseDB/fe/mysql_ssl_default_certificate/`下。密钥证书文件的生成请参考[密钥证书配置](../admin-manual/certificate.md)。

#### 停止 FE 节点

LakehouseDB FE 的停止可以通过下面的命令完成
...
我们进入到 `apache-lakehouseDB-x.x.x/be` 目录
...
我们上面完成了建表，输数据导入，下面我们就可以体验 LakehouseDB 的数据快速查询分析能力。
...
到这里我们整个快速开始就结束了，我们从 LakehouseDB 安装部署、启停、创建库表、数据导入及查询，完整的体验了LakehouseDB的操作流程，下面开始我们 LakehouseDB 使用之旅吧。
